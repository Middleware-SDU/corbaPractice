package cn.sdu.edu.middleware.excise2_2.daos.beans;


/**
* Beans/SubjectsHelper.java .
* Generated by the IDL-to-Java compiler (portable), version "3.2"
* from H:/repositoy/corbaPractice/src/main/java/cn/sdu/edu/middleware/excise2_2/daos/dao.idl
* Thursday, November 28, 2013 1:28:44 PM CST
*/

abstract public class SubjectsHelper
{
  private static String  _id = "IDL:Beans/Subjects:1.0";

  public static void insert (org.omg.CORBA.Any a, cn.sdu.edu.middleware.excise2_2.daos.beans.Subject[] that)
  {
    org.omg.CORBA.portable.OutputStream out = a.create_output_stream ();
    a.type (type ());
    write (out, that);
    a.read_value (out.create_input_stream (), type ());
  }

  public static cn.sdu.edu.middleware.excise2_2.daos.beans.Subject[] extract (org.omg.CORBA.Any a)
  {
    return read (a.create_input_stream ());
  }

  private static org.omg.CORBA.TypeCode __typeCode = null;
  synchronized public static org.omg.CORBA.TypeCode type ()
  {
    if (__typeCode == null)
    {
      __typeCode = cn.sdu.edu.middleware.excise2_2.daos.beans.SubjectHelper.type ();
      __typeCode = org.omg.CORBA.ORB.init ().create_sequence_tc (0, __typeCode);
      __typeCode = org.omg.CORBA.ORB.init ().create_alias_tc (cn.sdu.edu.middleware.excise2_2.daos.beans.SubjectsHelper.id (), "Subjects", __typeCode);
    }
    return __typeCode;
  }

  public static String id ()
  {
    return _id;
  }

  public static cn.sdu.edu.middleware.excise2_2.daos.beans.Subject[] read (org.omg.CORBA.portable.InputStream istream)
  {
    cn.sdu.edu.middleware.excise2_2.daos.beans.Subject value[] = null;
    int _len0 = istream.read_long ();
    value = new cn.sdu.edu.middleware.excise2_2.daos.beans.Subject[_len0];
    for (int _o1 = 0;_o1 < value.length; ++_o1)
      value[_o1] = cn.sdu.edu.middleware.excise2_2.daos.beans.SubjectHelper.read (istream);
    return value;
  }

  public static void write (org.omg.CORBA.portable.OutputStream ostream, cn.sdu.edu.middleware.excise2_2.daos.beans.Subject[] value)
  {
    ostream.write_long (value.length);
    for (int _i0 = 0;_i0 < value.length; ++_i0)
      cn.sdu.edu.middleware.excise2_2.daos.beans.SubjectHelper.write (ostream, value[_i0]);
  }

}
